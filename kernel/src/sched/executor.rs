...
/// Per-operator time budget configuration.
#[derive(Debug, Clone, Copy)]
pub struct OperatorBudget {
    /// Budget for elementwise ops (Relu, Add, etc.) in microseconds.
    pub elementwise_us: u64,
    /// Budget for reduction ops (Softmax, ReduceMean) in microseconds.
    pub reduction_us: u64,
    /// Budget for GEMM ops (MatMul, Conv) in microseconds.
    pub gemm_us: u64,
    /// Budget for attention ops in microseconds.
    pub attention_us: u64,
    /// Budget for GPU kernels in microseconds.
    pub gpu_kernel_us: u64,
    /// Soft limit multiplier (warning at 1x, soft at this, hard at hard_multiplier).
    pub soft_multiplier: u64,
    /// Hard limit multiplier (abort inference at this multiple of budget).
    pub hard_multiplier: u64,
}

impl OperatorBudget {
    /// Default budgets per the spec.
    pub const DEFAULT: Self = Self {
        elementwise_us: 1_000,    // 1 ms
        reduction_us: 10_000,     // 10 ms
        gemm_us: 100_000,         // 100 ms
        attention_us: 500_000,    // 500 ms
        gpu_kernel_us: 1_000_000, // 1000 ms
        soft_multiplier: 2,
        hard_multiplier: 10,
    };
}

/// Operator type for budget lookup.
...
pub enum BudgetResult {
    ...
    Ok,
    /// Exceeded 1x budget — log warning.
    Warning,
    /// Exceeded soft limit — emit metric, flag for profiling.
    SoftLimit,
    /// Exceeded hard limit — abort inference.
    HardLimit,
}

impl OperatorBudget {
    /// Check an operator's actual execution time against its budget.
    pub fn check(&self, op_class: OperatorClass, actual_us: u64) -> BudgetResult {
        let budget = match op_class {
