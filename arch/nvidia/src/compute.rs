...
//! Compute engine — kernel launch, grid/block config, synchronization.
//!
//! Provides a stub compute engine that manages kernel lifecycle without
//! touching real GPU hardware.  Kernels follow the state machine:
...
impl Dim3 {
    ...
    /// Total number of threads/blocks represented by this `Dim3`.
...
pub struct LaunchConfig {
    ...
    /// Stream/queue ordinal.
    pub stream: u32,
}

/// Maximum threads per block (hardware limit across Maxwell–Blackwell).
const MAX_THREADS_PER_BLOCK: u64 = 1024;

/// Maximum dynamic shared memory per block (160 KiB).
const MAX_SHARED_MEMORY: u32 = 163840;

impl LaunchConfig {
    /// Validate that this configuration is within hardware limits.
    pub fn validate(&self) -> Result<(), GpuError> {
        if self.block.total() > MAX_THREADS_PER_BLOCK {
...
pub enum KernelStatus {
    ...
    Queued,
    /// Being prepared for dispatch (reserved for future hardware handshake).
    Launching,
...
impl ComputeEngine {
    ...
    ///
    /// In this stub implementation we skip the `Launching` state.
...
mod tests {
    ...
    // 4. Launch config validation (block too large).
    #[test]
    fn test_launch_config_block_too_large() {
    ...
    // 6. Launch config shared memory limit.
    #[test]
    fn test_launch_config_shared_memory_limit() {
        let cfg = LaunchConfig {
            grid: Dim3::linear(1),
            block: Dim3::linear(256),
    ...
    // 16. Zero block dimension rejected.
    #[test]
    fn test_zero_block_dimension() {
        let cfg = LaunchConfig {
            grid: Dim3::linear(1),
    ...
    // 17. Exactly MAX_THREADS_PER_BLOCK is allowed.
    #[test]
    fn test_exact_max_threads() {
        let cfg = LaunchConfig {
            grid: Dim3::linear(1),
            block: Dim3::new(32, 32, 1), // 1024 == MAX_THREADS_PER_BLOCK
            shared_memory: 0,
            stream: 0,
        };
    ...
    // 18. Exactly MAX_SHARED_MEMORY is allowed.
    #[test]
