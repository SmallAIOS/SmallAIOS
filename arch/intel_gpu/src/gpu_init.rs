...
pub enum GpuState {
    ...
    Uninitialized,
    /// MMIO / GTT aperture mapped into the address space.
    BarsMapped,
...
pub enum PowerState {
    ...
    LowPower,
    /// Deep sleep (RC6p/RC6pp) -- minimal leakage.
    Suspended,
}
...
pub struct GpuRegisters {
    /// MMIO base: GPU control/status registers.
    pub mmio_base: u64,
    /// GTT/Aperture base: GPU-accessible memory window.
    pub gtt_base: u64,
    /// Size of MMIO region in bytes.
    pub mmio_size: u64,
    /// Size of GTT aperture in bytes.
    pub gtt_size: u64,
}
...
pub struct InitConfig {
    /// Enable the compute engine (render/compute command streamer).
    pub enable_compute: bool,
    /// Enable the copy engine (blitter command streamer).
...
impl GpuContext {
    ...
    /// Map MMIO registers and GTT aperture from the PCI device's BARs.
    ///
    /// Expects at least two memory-type BARs. The first becomes the MMIO
    /// region and the second becomes the GTT aperture.
    ///
    ...
    pub fn map_bars(&mut self, bars: &[BaseAddressRegister]) -> Result<(), GpuError> {
        ...
        self.registers = Some(GpuRegisters {
            ...
            gtt_base: memory_bars[1].address,
            mmio_size: memory_bars[0].size,
            gtt_size: memory_bars[1].size,
        });
    ...
    ///
    /// On real hardware this programs the render/compute command streamer
    /// and blitter; currently a state-machine stub.
    ///
    ...
    pub fn initialize_engines(&mut self) -> Result<(), GpuError> {
        ...
        // 1. Reset render/compute command streamer via GDRST
        // 2. Initialize GuC/HuC firmware
        // 3. Configure blitter command streamer
        // 4. Set up hardware contexts
    ...
    /// Suspend the GPU (power gate / RC6).
    ///
...
mod tests {
    use super::*;
    use crate::gpu_id::{GpuArchitecture, GpuGeneration, GpuInfo};
    use crate::pcie::{BarType, BaseAddressRegister};
    ...
    fn map_bars_transitions_to_bars_mapped() {
        ...
        assert_eq!(regs.gtt_base, 0x3800_0000_0000);
    }
