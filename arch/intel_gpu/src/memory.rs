...
//! GPU local memory allocator.
//!
//! Manages two regions of local memory (or system memory for iGPUs):
...
/// GPU page size -- 64 KiB, the native page granularity for Intel GPUs.
...
/// Describes which memory region an allocation belongs to.
...
pub struct GpuAllocation {
    ...
    pub id: u64,
    /// Byte offset within the memory region.
    pub offset: u64,
...
/// Bump-style memory allocator with static and dynamic regions.
#[derive(Clone, Debug, PartialEq)]
...
impl LocalMemoryAllocator {
    ...
    /// `static_fraction` -- fraction reserved for the static region
...
mod tests {
    ...
    fn test_new_allocator_splits_correctly() {
        let alloc = LocalMemoryAllocator::new(ONE_GIB, 0.7);
    ...
    fn test_alloc_static() {
        let mut alloc = LocalMemoryAllocator::new(ONE_GIB, 0.7);
    ...
    fn test_out_of_memory() {
        let mut alloc = LocalMemoryAllocator::new(PAGE * 2, 0.5);
    ...
    fn test_used_free_tracking() {
        let mut alloc = LocalMemoryAllocator::new(ONE_GIB, 0.5);
    ...
    fn test_max_allocations() {
        ...
        let mut alloc = LocalMemoryAllocator::new(big, 1.0);
    ...
    fn test_full_static_fraction() {
        let alloc = LocalMemoryAllocator::new(ONE_GIB, 1.0);
    ...
    fn test_zero_static_fraction() {
        let alloc = LocalMemoryAllocator::new(ONE_GIB, 0.0);
