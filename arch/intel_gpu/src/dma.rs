...
//!
//! Manages asynchronous memory transfers between host RAM and device local
//! memory (and device-to-device copies). Uses the Intel blitter command
//! streamer (BCS) on real hardware. Each transfer goes through a strict state
...
pub enum DmaDirection {
    /// Host RAM -> Device local memory.
    HostToDevice,
    /// Device local memory -> Host RAM.
    DeviceToHost,
    /// Device -> Device (same GPU or peer).
    DeviceToDevice,
}
...
pub enum DmaStatus {
    ...
    Pending,
    /// Currently being executed by the blitter engine.
    InProgress,
...
impl DmaEngine {
    ...
    /// Number of transfers currently in Pending state.
    ...
    /// Number of transfers currently in InProgress state.
