...
- [ ] 3.2 Cross-validate every message round-trip against fixtures generated by `audit-export/tests/scripts/gen_fixtures.rs` (a Rust binary talking to a live immudb sidecar; checked into `audit-export/tests/proof_vectors/`) *(deferred — requires live immudb; falls into Phase 5 with the verifier vectors; SmallAIOS stays Rust-only)*
...
- [ ] 5.1 Implement `TxHeader.eh` recomputation (SHA-256 over `kvDigest`s) *(wire-byte layout pending live immudb fixture; `recompute_alh_v1_pending()` returns `VerifyError::AlhWirePending`)*
...
- [ ] 5.7 Generate ≥20 inclusion vectors via Rust harness `audit-export/tests/scripts/gen_fixtures.rs` (uses this crate's own gRPC client against a developer-controlled immudb sidecar); check into `audit-export/tests/proof_vectors/inclusion_v1_*.bin` *(requires live immudb; no Go anywhere in the repo)*
