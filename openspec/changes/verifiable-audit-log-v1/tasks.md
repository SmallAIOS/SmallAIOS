...
- [ ] 5.1 Implement `TxHeader.eh` recomputation (SHA-256 over `kvDigest`s)
- [ ] 5.2 Implement `Alh` recomputation `(id, ts, prevAlh, eh)`
- [ ] 5.3 Implement inclusion-proof walker; verify each entry's leaf against the recomputed root
- [ ] 5.4 Implement dual / linear consistency proof walker `(sourceTxId → targetTxId)`
- [ ] 5.5 Implement Ed25519 signature verification against pinned `tls.server_pubkey_fingerprint`
- [ ] 5.6 Implement cold-start rollback detection: `currentState.txId < local last_state.txId` ⇒ halt
- [ ] 5.7 Generate ≥20 inclusion vectors via Go-SDK harness `tests/scripts/gen_fixtures.go`; check into `tests/proof_vectors/inclusion_v1_*.bin`
