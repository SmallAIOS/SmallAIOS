...
- [~] 2.10 Create `arch/aarch64/src/tegra234_uart.rs` — Tegra Combined UART (TCU) MMIO driver at `0x0c280000`. NS16550-compatible register layout. Polling write_byte initially; interrupt-driven later. **Partial — landed as a DRAM-stash placeholder in sub-PR 2e.** Both UART_1 (`0x0310_0000`) and the TCU mailbox (`0x0C28_0000`) were tried during 2e bring-up and produced fabric-level errors (SError EC=0x2F routed to EL3, and Sync Exception translation faults respectively) when accessed from our EL2 context post-`ExitBootServices`. NVIDIA's UEFI reaches the console through a privileged path (likely an SMC into TF-A or a TrustZone-only MMIO route) that we don't yet replicate. Real kernel-side UART output is **deferred to a follow-up sub-PR** (2e.1 or similar) that will need DTB parsing, our own page tables, or an SMC into TF-A's vendor namespace.
- [~] 2.11 Wire `tegra234_uart` into the `console` module under the `tegra234` feature, replacing the X1 `uart`. **Partial — `uart::init`/`uart::putc` now dispatch to `tegra234_uart` (replacing the prior `tcu_stub`).** Output goes to the DRAM placeholder for now; the dispatch wiring is what the follow-up real driver plugs into.
...
> **Sub-PR 2e (ExitBootServices + kernel handoff)** — extends task 2.7's `efi_main` with the actual `GetMemoryMap` + `ExitBootServices` retry loop and the tail-call to `kernel_main(dtb)`. Hand-rolled `MemoryDescriptor` + `BootServices` fn-pointer fields at spec-required offsets land here too. **Verified on Orin NX hardware (P3767-0000 + P3768-0000, JetPack 6.2.1)**: GetMemoryMap success with map_size=0x6C0, desc_size=0x30; ExitBootServices(map_key) succeeds; `kernel_main` reaches its idle loop without exception. No observable serial output post-EBS yet — see the deferred half of 2.10 above.
>
> **Sub-PR 2e-output (interim: kernel-side output via `con_out`)** — proves the kernel's banner / DTB / memory-detection / heap-init / idle-loop path runs end-to-end on real Orin hardware by **deliberately skipping ExitBootServices** and routing `tegra234_uart::putc` through `SystemTable.con_out`. UEFI services persist (so this is *not* a real kernel boot) but every line of kernel output reaches the J-class carrier's TTL header, and we get on-board confirmation that everything except the post-EBS UART works. The fix for the post-EBS UART (DTB parse / page tables / SMC) is the next sub-PR; once that's in, we re-instate the ExitBootServices call and remove this interim mode.
>
> Two follow-ups uncovered while landing 2e-output:
>   1. `smallaios-kernel::VERSION` was a hardcoded `"0.1.0"`; switched to `env!("CARGO_PKG_VERSION")` so it tracks `cargo-release` bumps automatically. (Fixed in 2e-output.)
>   2. `kernel::mem::phys::parse_dtb` doesn't recognize Tegra234's memory-node layout — `[mem]  DTB parsed: 0 region(s), 0 MiB usable RAM` on real hardware even though the DTB pointer is correct. The parser was written for QEMU virt + Tegra X1; Tegra234's `/reserved-memory/` carve-outs and multi-cell `reg = <…>` formats need additional handling. Tracked as a separate follow-up.
...
- [x] 2.18 Document the `dd if=build-jetson-usb.img of=/dev/sdX bs=4M status=progress` workflow in `docs/jetson-orin-uefi-boot.md`, with a prominent warning to confirm the target device. **Landed in sub-PR 2g.** Doc shows the macOS `diskutil unmountDisk` + `dd if=… of=/dev/rdiskN bs=4m` and Linux `dd if=… of=/dev/sdX bs=4M status=progress conv=fdatasync` variants; a "⚠️ WARNING — `dd` is destructive" call-out sits inline.

### 2g. Boot procedure docs (the primary user surface)

- [x] 2.19 Create `docs/jetson-orin-uefi-boot.md` covering: prerequisites (USB stick ≥256 MB, USB-to-TTL serial cable connected to the J4012 UART header, terminal emulator configured at the documented baud rate), the `dd` step with target-device confirmation guidance, the J4012 firmware-menu key sequence (likely Esc at boot — confirm against Seeed J4012 docs), the UEFI Secure Boot disable-or-enroll step, the expected serial-console output, and the recovery procedure ("remove stick, power-cycle, reboot to L4T"). **Landed in sub-PR 2g.** Doc covers prerequisites (hardware + build host + UEFI config), one-shot quickstart, expected serial output (lifted from the captured 2e-output evidence), forcing the UEFI boot menu (F11/Esc), recovery, troubleshooting, and a "what this does NOT do" section. README's deployment-matrix row updated to point at it.
- [x] 2.20 Add a one-paragraph "if you accidentally bricked the box" section linking to NVIDIA's SDK Manager + USB-recovery procedure — even though Phase 2 itself can't brick the box, an `efibootmgr` follow-up could. **Landed in sub-PR 2g** (in the "Recovery / unbricking" section of the doc).
