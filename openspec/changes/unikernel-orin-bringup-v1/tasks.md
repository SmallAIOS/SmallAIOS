...
- [ ] 1.4 Verify the recipe end-to-end on a real Orin NX (P3767-0000 / R36.4.7 host or equivalent) — capture the full boot output and paste in the PR description as the Phase 1 acceptance evidence. Acceptable execution modes: (a) Mac-as-build-host + Orin-NX-as-runner via SSH_HOST; (b) x86 Ubuntu host + Orin-NX-as-runner via SSH_HOST; (c) Orin-NX-as-both via the no-arg recipe variant. Document which mode produced the captured evidence. **Deferred to a follow-up commit** when the user is on a Rust-equipped build host (Mac or x86 Ubuntu); the Orin runner side is fully wired and waits only for a kernel binary to arrive via scp.
