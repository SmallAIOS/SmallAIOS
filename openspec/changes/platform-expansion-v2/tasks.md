...
- [ ] 15.1 Create `smallaios-bus` crate with feature flags (can, arinc429, arinc664, mil1553, spacewire, ccsds)
...
## 21. RISC-V Architecture Support

- [ ] 21.1 Create `smallaios-arch-riscv64` crate with target riscv64gc-unknown-none-elf
- [ ] 21.2 Create custom target JSON and linker script for RISC-V bare metal
- [ ] 21.3 Implement assembly entry point (set stack, clear BSS, set stvec, call kernel_main)
- [ ] 21.4 Implement SV48 4-level page table management (map, unmap, protect)
- [ ] 21.5 Implement TLB flush via sfence.vma instruction
- [ ] 21.6 Implement PLIC interrupt controller driver (priority, enable, claim, complete)
- [ ] 21.7 Implement CLINT timer driver (mtime/mtimecmp for periodic tick)
- [ ] 21.8 Implement SBI HSM extension calls for SMP boot (hart_start, hart_stop, hart_get_status)
- [ ] 21.9 Implement SBI IPI extension for inter-hart interrupts
- [ ] 21.10 Implement NS16550A UART driver (QEMU virt machine console output)
- [ ] 21.11 Implement CPU feature detection (A, C, D, F extensions)
- [ ] 21.12 Verify boot-to-serial-output in QEMU virt (riscv64)
- [ ] 21.13 Add RISC-V to CI (build + QEMU smoke test)
- [ ] 21.14 Add RISC-V HAL implementation to kernel HAL trait

## 22. SoC FPGA Platform Support

- [ ] 22.1 Create `smallaios-fpga` crate with feature flags (zynq, polarfire)
- [ ] 22.2 Implement AXI4-Lite memory-mapped register read/write driver
- [ ] 22.3 Implement AXI4 full burst transfer driver
- [ ] 22.4 Implement AXI DMA controller driver (simple mode)
- [ ] 22.5 Implement AXI DMA scatter-gather mode
- [ ] 22.6 Implement DTB-based peripheral discovery for FPGA soft-IP blocks
- [ ] 22.7 Implement interrupt routing from FPGA fabric to CPU interrupt controller
- [ ] 22.8 Implement Zynq UltraScale+ platform support package (PS-PL interface, clocks, resets)
- [ ] 22.9 Implement PolarFire SoC platform support package (RISC-V + FPGA fabric)
- [ ] 22.10 Unit tests for AXI register access and DMA transfers (mock MMIO)
- [ ] 22.11 Integration test on Zynq board: read/write FPGA registers from SmallAIOS

## 23. Deployment and Provisioning (Phase 11 Revision)

- [ ] 23.1 Implement UEFI Secure Boot support with ML-DSA-65 signed kernel image
- [ ] 23.2 Implement PXE/iPXE bare metal network boot provisioning
- [ ] 23.3 Implement VM image generation (raw disk, qcow2, VMDK formats)
- [ ] 23.4 Implement image signing with ML-DSA-65 post-quantum signatures
- [ ] 23.5 Update OCI image build for multi-arch (amd64, arm64, riscv64)
- [ ] 23.6 Verify image size target < 15 MB (kernel + runtime, no model)

## 24. IPC Transport Extensions

- [ ] 24.1 Extend ZenohRouter with bus transport registration interface
- [ ] 24.2 Implement transport auto-discovery from DTB/ACPI peripheral enumeration
- [ ] 24.3 Implement cross-transport routing (message on CAN routed to TCP subscriber)
- [ ] 24.4 Unit tests for transport-agnostic pub/sub across mixed transports
- [ ] 24.5 Integration test: inference result published over TCP, delivered over CAN

## 25. HAL Extensions

- [ ] 25.1 Define BusPeripheral HAL trait (init, tx_frame, rx_frame, irq_handler)
- [ ] 25.2 Define FpgaFabric HAL trait (axi_read, axi_write, dma_transfer)
- [ ] 25.3 Implement RISC-V HAL (paging, PLIC, CLINT, SBI wrappers)
- [ ] 25.4 Update hardware platform spec with RISC-V and SoC FPGA tier 2 targets
- [ ] 25.5 Add riscv64gc-unknown-none-elf build target to Makefile and CI

## 26. Benchmark Infrastructure

- [ ] 26.1 Create bench/ directory structure (scripts, configs, models, results)
- [ ] 26.2 Implement cold start measurement harness (power-on to first inference)
- [ ] 26.3 Implement warm inference latency harness (N=1000+ runs, p50/p99/p999)
- [ ] 26.4 Implement throughput benchmark (batch sizes 1, 4, 16, 64)
- [ ] 26.5 Implement jitter measurement (stdev and max deviation)
- [ ] 26.6 Implement memory footprint measurement (peak RSS)
- [ ] 26.7 Download and prepare benchmark models: MobileNetV2 (vision), DistilBERT (text), Whisper-tiny (audio/signal)
- [ ] 26.8 Create Linux bare metal baseline scripts (ONNX Runtime C++ binary)
- [ ] 26.9 Create Docker baseline (Dockerfile + ONNX Runtime)
- [ ] 26.10 Create K8s/K3s baseline (deployment manifests + ONNX Runtime pod)
- [ ] 26.11 Create hardware-specific configs (DGX Spark, Xeon, Jetson, RPi)
- [ ] 26.12 Document BIOS settings, CPU frequency pinning, thermal monitoring
- [ ] 26.13 Implement report generation (markdown tables, CSV export)
- [ ] 26.14 Run benchmark suite on all 4 hardware targets, generate comparison report

## 27. Formal Verification — Bus Protocols

- [ ] 27.1 Write TLA+ model for CAN bus arbitration (priority-based, no starvation)
- [ ] 27.2 Write TLA+ model for ARINC 429 transmit scheduler (no label starvation)
- [ ] 27.3 Write TLA+ model for AFDX Virtual Link BAG enforcement (no deadline miss)
- [ ] 27.4 Write TLA+ model for MIL-STD-1553 command/response (no unanswered commands)
- [ ] 27.5 Write TLA+ model for SpaceWire link state machine (correct transitions)
- [ ] 27.6 Create TLC configs for all bus protocol models
- [ ] 27.7 Add bus protocol TLA+ verification to CI
